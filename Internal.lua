-- ==========================================================
-- INTERNAL | UNNAMED GOD EDITION (FINAL REPAIR)
-- UPDATED: UI Sync / Camera Lock / Movement Fix
-- ==========================================================

local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
local UserInputService = game:GetService("UserInputService")
local LocalPlayer = Players.PlayerGui.Parent
local Mouse = LocalPlayer:GetMouse()
local Camera = workspace.CurrentCamera

-- 重複防止
if game.CoreGui:FindFirstChild("UnnamedGodFinal") then
    game.CoreGui:FindFirstChild("UnnamedGodFinal"):Destroy()
end
...
-- --- UI構築 (カテゴリー別) ---
local ScreenGui = Instance.new("ScreenGui", game.CoreGui); ScreenGui.Name = "UnnamedGodFinal"
...
local function CreateSection(text, y)
    ...
    l.BackgroundTransparency = 1; l.Text = "--- " .. text .. " ---"; l.TextColor3 = Color3.fromRGB(255, 0, 80); l.Font = Enum.Font.Code; l.TextSize = 16; return y + 45
end

-- トグル作成用
local function AddToggle(text, key, y)
    local btn = Instance.new("TextButton", Content)
    btn.Size = UDim2.new(1, -10, 0, 38); btn.Position = UDim2.new(0, 5, 0, y); btn.BackgroundColor3 = Color3.fromRGB(25, 25, 25); btn.BorderSizePixel = 0; btn.Font = Enum.Font.Code; btn.TextSize = 14; btn.TextXAlignment = Enum.TextXAlignment.Left
    local function up() btn.Text = "  [" .. (S[key] and "ON" or "OFF") .. "] " .. text; btn.TextColor3 = S[key] and Color3.new(1, 0.2, 0.4) or Color3.new(0.6, 0.6, 0.6) end
    btn.MouseButton1Click:Connect(function() S[key] = not S[key]; up() end); up(); return y + 42
end
...
local function AddInput(text, key, y)
    ...
    b.FocusLost:Connect(function() local v = tonumber(b.Text); if v then S[key] = v end b.Text = tostring(S[key]) end); return y + 42
end
...
RunService.Stepped:Connect(function()
    ...
    if S.Underground then
        ...
        -- カメラ焦点を強制的に地上(+3)へ固定し、視点が下がるのを防ぐ
...
function ApplyFly()
    ...
    task.spawn(function()
        ...
        _G.FlyLoop = RunService.Heartbeat:Connect(function()
            if not S.Fly or not root.Parent then bv:Destroy(); _G.FlyLoop:Disconnect(); return end
            local dir = Vector3.new(0,0,0)
            if UserInputService:IsKeyDown(Enum.KeyCode.W) then dir += Camera.CFrame.LookVector end
            if UserInputService:IsKeyDown(Enum.KeyCode.S) then dir -= Camera.CFrame.LookVector end
            bv.Velocity = dir.Unit * S.FlySpeed
        end)
    end)
end
